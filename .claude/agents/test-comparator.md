...
- **End-of-file checkpoint** — a final `## Local verification & commit` section with:
  1. From the repo root (substituting `<lang>` ∈ {java, dotnet, typescript}), run `gh optivem test system --system-config docker/<lang>/monolith/systems.json --test-config system-test/<lang>/tests-latest.json` (latest suite) and `gh optivem test system --system-config docker/<lang>/monolith/systems.json --test-config system-test/<lang>/tests-legacy.json` (legacy suite). **Never** substitute raw language toolchain commands (`./gradlew test`, `dotnet test`, `npm test`, `npx playwright test`) — `gh optivem test system` is the only supported entry point because it manages containers and config.
