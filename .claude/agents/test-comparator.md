...
- **End-of-file checkpoint** — a final `## Local verification & commit` section with:
  1. From the repo root (substituting `<lang>` ∈ {java, dotnet, typescript}), set `GH_OPTIVEM_CONFIG` once per shell and run the verbs without per-flag overrides. Latest: `$env:GH_OPTIVEM_CONFIG = "gh-optivem-monolith-<lang>.yaml"; gh optivem system start; gh optivem test setup; gh optivem test run`. Legacy: `$env:GH_OPTIVEM_CONFIG = "gh-optivem-monolith-<lang>-legacy.yaml"; gh optivem test setup; gh optivem test run`. **Never** substitute raw language toolchain commands (`./gradlew test`, `dotnet test`, `npm test`, `npx playwright test`) — `gh optivem test run` is the only supported entry point because it manages containers and config.
