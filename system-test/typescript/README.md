...
```bash
gh optivem run system --system-config docker/typescript/monolith/systems.json
```

Run all latest test suites:

```bash
gh optivem test system --system-config docker/typescript/monolith/systems.json --test-config system-test/typescript/tests-latest.json
```

Run legacy test suites:

```bash
gh optivem test system --system-config docker/typescript/monolith/systems.json --test-config system-test/typescript/tests-legacy.json
```

Run only sample tests (one per suite, fast smoke):

```bash
gh optivem test system --system-config docker/typescript/monolith/systems.json --test-config system-test/typescript/tests-latest.json --sample
```

Run a specific suite by ID:

```bash
gh optivem test system --system-config docker/typescript/monolith/systems.json --test-config system-test/typescript/tests-latest.json --suite acceptance-api
```

Rebuild container images before bringing the system up:

```bash
gh optivem build system --system-config docker/typescript/monolith/systems.json
```
...
Substitute `docker/typescript/multitier/systems.json` for the multitier architecture.
